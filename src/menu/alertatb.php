    ...
    case 'con_alt':
        if (isset($_SESSION['id'])) {
            ...
            try {
                ...
                FROM (SELECT COUNT(*) AS total_count FROM tb_alerta WHERE estado = 1 AND tipo_alerta IN ('IVE', 'PASS') UNION ALL
                SELECT COUNT(*) AS total_count FROM ahomcrt crt INNER JOIN tb_cliente tc ON tc.idcod_cliente = crt.ccodcli 
                WHERE crt.fec_ven > CURDATE() AND crt.fec_ven < DATE_ADD(CURDATE(), INTERVAL 15 DAY)) AS con");
