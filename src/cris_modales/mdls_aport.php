...
function BuscarCli()
{
    ...
    $consulta2 = mysqli_query($conexion, "SELECT idcod_cliente, no_identifica,short_name compl_name, date_birth,no_tributaria, 
                                            ...
                                            FROM `tb_cliente` WHERE id_tipoCliente != 'JURIDICO' AND idcod_cliente IN (SELECT ccodcli FROM aprcta WHERE estado='A') AND estado=1");
    while ($registro = mysqli_fetch_array($consulta2, MYSQLI_ASSOC)) {
        // $nombres = utf8_encode($registro["nombres"]);
        // $apellido = utf8_encode($registro["apellido"]);

        $idcod_cliente = ($registro["idcod_cliente"]);
        $no_identifica = ($registro["no_identifica"]);
        $nit = ($registro["no_tributaria"]);
        $compl_name = utf8_encode($registro["compl_name"]);
...
<div class="modal" id="findaportcta">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            <!-- Modal Header -->
            <div class="modal-header">
                <h4 class="modal-title">Busqueda de cuentas de aportaciones</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <!-- Modal body -->
            <div class="modal-body">
