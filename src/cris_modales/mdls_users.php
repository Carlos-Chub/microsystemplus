...
function BuscarUsers($conexion, $id_actual)
{
    $consulta2 = mysqli_query($conexion, "SELECT 
    us.id_usu AS id_usuario, CONCAT(us.nombre,' ', us.apellido) AS nombre,tr.id AS id_cargo,cg.UsuariosCargoProfecional AS cargo,ag.nom_agencia AS nombreagen,ag.cod_agenc AS codagen
    FROM tb_usuario us
    INNER JOIN  `clhpzzvb_bd_general_coopera`.tb_usuarioscargoprofecional cg ON us.puesto = cg.id_UsuariosCargoProfecional
    INNER JOIN  `clhpzzvb_bd_general_coopera`.tb_rol tr ON cg.id_UsuariosCargoProfecional = tr.siglas
                ...
                <div class="table-responsive">
                    <table id="tabla_nomusuarios" class="table table-striped table-hover"
                        ...
                        <tbody id="tabla_nomusuarios">
                            <?php
                            BuscarUsers($conexion, $_SESSION['id']);
                            ?>
                        </tbody>
                    </table>
                </div>
