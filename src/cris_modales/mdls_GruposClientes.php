...
function grupos()
{
    ...
    // $consulta2 = mysqli_query($conexion, "SELECT gru.id_grupos, gru.codigo_grupo,gru.NombreGrupo, gru.direc, gru.id_grupos, gru.canton, gru.Depa, gru.Muni, muni.nombre, gru.estadoGrupo FROM tb_grupo AS gru INNER JOIN clhpzzvb_bd_general_coopera.municipios AS muni ON gru.Muni = muni.codigo_municipio WHERE gru.estado = 1");
    ...
    INNER JOIN clhpzzvb_bd_general_coopera.municipios AS muni ON gru.Muni = muni.codigo_municipio WHERE gru.estado = 1");
    ...
    while ($registro = mysqli_fetch_array($consulta2, MYSQLI_ASSOC)) {
        ...
        $NombreGrupo = ($registro["NombreGrupo"]);
        //$fecha_sys = utf8_encode($registro["fecha_sys"]);
 ...
 <tr> 
          ...
          <td>' . $NombreGrupo . '</td>
          <td>' . $direc . '</td>
          <td> <button type="button" class="btn btn-success" onclick="instgrp(&apos;' . $codigo_grupo . '&apos;, &apos;' . $NombreGrupo . '&apos;,&apos;' . $direc . '&apos;, &apos;' . $id_grupos . '&apos; ,  &apos;' . $Muni . '&apos;, &apos;' . $Depa . '&apos; , &apos;' . $canton . '&apos;, &apos;' . $nomMuni . '&apos;, &apos;' . $estadoGrupo . '&apos;, &apos;' . $control . '&apos;)" >Aceptar</button> </td>
 </tr> ';
    }
}
?>
