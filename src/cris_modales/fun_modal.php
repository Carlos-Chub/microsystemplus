        ...
        case 'gruposcredito':
            ...
            if ($status1 != "all") {
                ...
                $query = "SELECT grup.id_grupos,grup.codigo_grupo,grup.NombreGrupo,grup.fecha_sys,grup.direc,cre.NCiclo,cre.Cestado,est.EstadoCredito estado FROM `tb_grupo` grup 
                        ...
                        INNER JOIN `clhpzzvb_bd_general_coopera`.`tb_estadocredito` est on est.id_EstadoCredito=cre.Cestado
            ...
            while ($fila = mysqli_fetch_array($consulta, MYSQLI_ASSOC)) {
                $array_datos[] = array(
                    ...
                    "3" => $fila["NCiclo"],
                    "4" => $fila["estado"],
                    "5" => '<button type="button" class="btn btn-success" data-bs-dismiss="modal" onclick="printdiv2( `#cuadro`,[`' . $fila["id_grupos"] . '`,' . $fila["NCiclo"] . ',`' . $fila["Cestado"] . '`])" >Aceptar</button>'
                );
                $i++;
                $contador++;
            }
            ...
            case 'grupos_a_solicitud':
                ...
                while ($fila = mysqli_fetch_array($consulta, MYSQLI_ASSOC)) {
                    $array_datos[] = array(
                        ...
                        "3" => $fila["NCiclo"],
                        "4" => $fila["estado"],
                        "5" => '<button type="button" class="btn btn-primary" data-bs-dismiss="modal" data-ccodcta="' . $fila["id_grupos"] . ' ' . $fila["NCiclo"] . '" onclick="grup_cambiar_solicitud(this)">Cambiar a Solicitud </button>'
                ...
                case 'credgrupo_for_delete':
                    ...
                    while ($fila = mysqli_fetch_array($consulta, MYSQLI_ASSOC)) {
                        $array_datos[] = array(
                            ...
                            "3" => $fila["NCiclo"],
                            "4" => $fila["estado"],
                            "5" => '<button type="button" class="btn btn-danger" data-ccodcta="' . $fila["id_grupos"] . ' ' . $fila["NCiclo"] . '" onclick="delete_cred_grup(this)"><i class="fas fa-trash-alt"></i> Eliminar</button>',
                            "6" => '<button type="button" class="btn btn-warning" data-ccodcta="' . $fila["id_grupos"] . ' ' . $fila["NCiclo"] . '" onclick="regre_cred_grup(this)"><i class="fas fa-arrow-left"></i> Cambiar a Aprobacion </button>'
