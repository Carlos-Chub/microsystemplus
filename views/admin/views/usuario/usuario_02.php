                ...
                <div class="card-body">
                                        ...
                                        <tbody id="tb_cuerpo_submenus" style="font-size: 0.9rem !important;">
                                            ...
                                            while ($row = mysqli_fetch_array($consulta, MYSQLI_ASSOC)) {
                                                ...
                                                if ($_SESSION['id'] == 4) { ?>
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <button type="button" class="btn btn-success btn-sm" onclick="printdiv5('id_usuario,id_usuario_past,usuario,cargo,nomagencia,codagencia/A,A,A,A,A,A/'+'/#/#/#/#',['<?= $id ?>','<?= $id ?>','<?= $nombre ?>','<?= $cargo ?>','<?= $nombreagen ?>','<?= $codagen ?>']);  recuperar_permisos('<?= $id ?>'); HabDes_boton(1);"><i class="fa-solid fa-eye"></i></button>
                                                            <button type="button" class="btn btn-danger btn-sm" onclick="eliminar('<?= $id ?>', 'crud_usuario', '0', 'delete_permisos')"><i class="fa-solid fa-trash"></i></button>
                                                        </td>
                                                        <th scope="row"><?= $id ?></th>
                                                        <td><?= $nombre ?></td>
                                                        <td><?= $cargo ?></td>
                    ...
                    <div class="container contenedort" style="max-width: 100% !important;">
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-sm-6">
                                <button type="button" class="btn btn-primary pt-3 pb-3 mb-2 mt-2 col-12 col-sm-12" onclick="abrir_modal('#modal_users', '#id_modal_hidden', 'id_usuario,usuario,cargo,nomagencia,codagencia/A,A,A,A,A/'+'/#/#/#/#')"><i class="fa-solid fa-magnifying-glass-plus me-2"></i>Buscar usuario</button>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-sm-6 col-md-4">
                                <div class="form-floating mb-3 mt-2">
                                    <input type="text" class="form-control" id="nomagencia" placeholder="Nombre de agencia" disabled>
                                    <label for="nomagencia">Nombre de agencia</label>
                                </div>
                            </div>
                            <div class="col-12 col-sm-6 col-md-4">
                                <div class="form-floating mb-3 mt-2">
                                    <input type="text" class="form-control" id="codagencia" placeholder="Código de agencia" disabled>
                        ...
                        <div class="row">
                            <div class="col-12 d-flex justify-content-end pe-4 mb-2">
                                <div class="form-check form-check-reverse">
                                    ...
                                    <input class="form-check-input" type="checkbox" name="todos" value="" id="todos" onclick="seleccionar_todos(this.checked)">
                                </div>
                            </div>
                        </div>
                        <!-- CONSULTA PARA APERTURAS TODOS LOS PERMISOS -->
                        <?php
                        $consulta = "SELECT td.id AS id_modulo, td.descripcion AS modulo, td.rama AS rama, tm.id AS id_menu, tm.descripcion AS menu, ts.id AS id_submenu,ts.condi AS condicion, ts.`file` AS archivo, ts.caption AS texto, ts.desarrollo FROM clhpzzvb_bd_general_coopera.tb_submenus ts
                        INNER JOIN clhpzzvb_bd_general_coopera.tb_menus tm ON ts.id_menu =tm.id
                        INNER JOIN clhpzzvb_bd_general_coopera.tb_modulos td ON tm.id_modulo =td.id
                        INNER JOIN clhpzzvb_bd_general_coopera.tb_permisos_modulos tbps ON td.id=tbps.id_modulo
                                                ...
                                                <div class="row border-bottom pb-2">
                                                    ...
                                                    <div class="col-2 col-md-2 mt-1">
                                                        <div class="d-flex justify-content-center">
                                                            <div class="form-check form-check-reverse">
                                                                <input class="form-check-input" type="checkbox" name="<?= "M-" . $alfabeto[$idcheckmod]; ?>" id="<?= "M" . $idcheckmod; ?>" onclick="seleccionar_checks(this.checked, '<?= $alfabeto[$idcheckmod]; ?>')">
                                                ...
                                                <div class="col-2 col-md-2 mt-1">
                                                    <div class="d-flex justify-content-center">
                                                        <div class="form-check">
                                                            <input class="form-check-input S" type="checkbox" value="<?= $registro[$i]['id_submenu']; ?>" name="<?= $alfabeto[$idcheckmod]; ?>" id="<?= "S_" . $registro[$i]['id_submenu']; ?>" onclick="desseleccionar_checks(this.checked, '<?= $alfabeto[$idcheckmod]; ?>')">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
...
<?php
            $consulta2 = "SELECT td.id AS id_modulo, td.descripcion AS modulo, td.rama AS rama, tm.id AS id_menu, tm.descripcion AS menu, ts.id AS id_submenu,ts.condi AS condicion, ts.`file` AS archivo, ts.caption AS texto, ts.desarrollo FROM clhpzzvb_bd_general_coopera.tb_submenus ts
                        INNER JOIN clhpzzvb_bd_general_coopera.tb_menus tm ON ts.id_menu =tm.id
                        INNER JOIN clhpzzvb_bd_general_coopera.tb_modulos td ON tm.id_modulo =td.id
                        INNER JOIN clhpzzvb_bd_general_coopera.tb_permisos_modulos tbps ON td.id=tbps.id_modulo
                        WHERE ts.estado='1' AND tm.estado='1' AND td.estado='1' AND tbps.estado='1' AND td.rama='A' AND 
                ...
                <div class="card text-bg-light mb-3">
                    ...
                    <div class="row border-bottom pb-2">
                        ...
                        <div class="col-2 col-md-2 mt-1">
                            <div class="d-flex justify-content-center">
                                <div class="form-check">
                                    <input class="form-check-input S" type="checkbox" value="<?= $registro2[$i]['id_submenu']; ?>" name="<?= $alfabeto[$idcheckmod]; ?>" id="<?= "S_" . $registro2[$i]['id_submenu']; ?>" onclick="desseleccionar_checks(this.checked, '<?= $alfabeto[$idcheckmod]; ?>')">
                                </div>
                            </div>
                        </div>
                    </div>
...
<div class="row justify-items-md-center">
    <div class="col align-items-center mt-2" id="modal_footer">
        <button type="button" class="btn btn-outline-success" id="btGuardar" onclick="guardar_editar_permisos('create_permisos')">
            <i class="fa-solid fa-floppy-disk"></i> Guardar
        </button>
        <button type="button" class="btn btn-outline-primary" id="btEditar" onclick="guardar_editar_permisos('update_permisos')">
