...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
    ...
    $qrydata = ["F",$rango]; // CREDITOS VIGENTES
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
    ...
    $qrydata =["G",$rango]; // para CANCELADOS
		$query = CreQUERY($qrydata, $db_name_general );
		$response = executequery($query, $fechas , $conexion);
		/* +++++++++++++++++++++++++++ VALIDACION DE DATOS +++++++++++++++++++++++++++++++++ */
    if (!$response[1]) {
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
	...
	$response = executequery($qryCli, $fechas2, $conexion);
	if (!$response[1]) {
	  echo json_encode(['status' => 0, 'mensaje' => $response[0]]);
	  return;
	}
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
		global $qryGaranti;
	$response = executequery($qryGaranti, $fechas2, $conexion);
	if (!$response[1]) {
    echo json_encode(['status' => 0, 'mensaje' => $response[0]]);
    return;
	}
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
		global $qryFiador;
  $response = executequery($qryFiador, $fechas2, $conexion);
  if (!$response[1]) {
    echo json_encode(['status' => 0, 'mensaje' => $response[0]]);
    return;
  }
...
function datosclientes($codcli, $datos, $vinculo)
{
    ...
    if (!(strlen($igss) >= 8 && strlen($igss) <= 20)) { $igss = ' '; }
    if (!(strlen($nit) >= 8 && strlen($nit) <= 20)) { $nit = ' '; }
    if (!(strlen($dpi) >= 8 && strlen($dpi) <= 20)) { $dpi = ' '; }
    // v
