...
if ($inputs[0] == "" && $inputs[1] == "") {
    echo json_encode(['status' => 0, 'mensaje' => 'Debe cargar una cuenta de aportaciones']);
    return;
}
...
if ($bandera) {
    echo json_encode(['status' => 0, 'mensaje' => 'Debe cargar una cuenta de ahorro válida']);
    return;
}
...
if ($radios[0] == "2") {
    ...
    if ($fecha_2 > $fecha_actual) {
        echo json_encode(['status' => 0, 'mensaje' => 'La fecha de hasta no puede ser mayor a la fecha de hoy']);
        return;
    }
    if ($fecha_1 > $fecha_2) {
        echo json_encode(['status' => 0, 'mensaje' => 'La fecha inicial no puede ser mayor a la fecha final']);
        return;
    }
}
...
if ($radios[0] == "1") {
    ...
    if ($fecha_2 != $fecha_actual && $fecha_1 != $fecha_actual) {
        echo json_encode(['status' => 0, 'mensaje' => 'Error en su solicitud']);
        return;
    }
}
$formato = $tipo;
...
$consulta = "SELECT * FROM aprmov WHERE cestado!=2 AND ccodaport = " . $ccodaho;
...
switch ($tip_report) {
    case "pdf": {
            ...
            class PDF extends FPDF
            {
                ...
                function Header()
                {
                    ...
                    $codcli = "";
                    $nit = "";
                    $datalib = mysqli_query($this->conexion, "SELECT `nlibreta`,`ccodcli`,`num_nit` FROM `aprcta` WHERE `ccodaport`=$this->ccodaho");
                    ...
                    $this->Cell(0, 10, 'Cuenta de aportacion:  ' . $this->ccodaho, 0, 1);
                    //TITULO DE REPORTE
                    $this->SetFillColor(204, 229, 255);
                    $this->Cell(0, 5, 'HISTORIAL DE CUENTA DE APORTACION', 0, 1, 'C', true);
    ...
    case "xlsx": {
            ...
            while ($mov = mysqli_fetch_array($data, MYSQLI_ASSOC)) {
                ...
                $hojaReporte->setCellValueByColumnAndRow(4, $linea, strval($numdoc));
