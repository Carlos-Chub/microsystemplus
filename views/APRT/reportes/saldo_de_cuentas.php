...
// echo json_encode(['Sesioasdfsadte', 0]);
//     return;
...
$consulta = "SELECT tp.nombre,cta.ccodaport, cl.short_name, calcular_saldo_apr_tipcuenta(cta.ccodaport,'$fecha_final') AS saldo, cta.tasa, cl.genero
FROM aprtip AS tp
INNER JOIN aprcta AS cta ON tp.ccodtip=SUBSTRING( cta.ccodaport ,7 , 2)  
...
switch ($tip_report) {
    case "pdf": {
            ...
            while ($rowdata = mysqli_fetch_array($data, MYSQLI_ASSOC)) {
                $bd_ccodaho = utf8_encode($rowdata["ccodaport"]);
