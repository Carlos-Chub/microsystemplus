...
switch ($condi) {
    case 'lib':
        ...
        $confirma = "0";
        $array[] = [];
        $noprint =  mysqli_query($conexion, "SELECT `dfecope`,`cnumdoc`,`ctipope`,`monto`,`numlinea`,`correlativo` FROM `aprmov` WHERE cestado!=2 AND `ccodaport`='$ccodaport' AND `nlibreta`=$libreta AND `lineaprint`='N'");
