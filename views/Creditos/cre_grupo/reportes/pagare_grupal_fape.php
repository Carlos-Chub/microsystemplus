...
$instcorto = utf8_decode($info[0]["nomb_cor"]);
...
function parr1($datacre, $vlrs, $registro)
{
    ...
    $datos = "<p>" . utf8_decode('En la ciudad de Guatemala, el día ') . fechletras($registro[0]['DFecDsbls']) . ', Nosotras: ';
    $i = 0;
    $nombresdetalles = '';
    ...
    $datos .= 'todas Guatemaltecas con domicilio en ' . utf8_decode($registro[0]['municipio']) . ', del departamento de ' . utf8_decode($registro[0]['departamento']) . '.';
    $datos .= ' Por medio del presente <vb>' . $vlrs[0] . '</vb>, "nos comprometemos a cancelar de manera incondicional y libre de protesto", a la orden o endoso de <vb>' . utf8_decode($vlrs[1]) . '</vb>';
    ...
    $datos .= '<vb>' . utf8_decode($vlrs[1] . '</vb> La cual nosotras: <vb>');
    $nombres = '';
    $i = 0;
    while ($i < count($registro)) {
    ...
    $datos .= $nombres;
    $datos .= utf8_decode('</vb> declaramos conocer dentro del horario comprendido de lunes a viernes de ocho a diecisiete horas. En caso hubiera necesidad de incurrir en gastos de cobro ya sea por la vía extrajudicial, como judicial, nosotras: ');
    $datos .= '<vb>' . $nombres .  utf8_decode('</vb> asumimos la total responsabilidad del pago de los mismos comprometiéndonos a cancelar el monto para tal efecto se nos cobre, incluyendo honorarios de abogados, renunciando desde ya al fuero de mi domicilio y, nos sometemos de manera expresa al fuero tribunal competente del departamento de Guatemala, por su parte las codeudoras: ');
    $datos .= $nombresdetalles;
    $datos .= utf8_decode(' Manifestamos por el presente documento se constituye en fiadores solidarios mancomunados respectivamente la obligación que contrae el presente ' . $vlrs[0] . ': ');
    $datos .= '<vb>' . $nombres . utf8_decode('</vb> ofreciendo responder en forma individual, conjunta e inmediatamente renunciando desde ya al fuero de cualquier otro domicilio, y de manera expresa nos sometemos al tribunal competente del departamento de Guatemala.');
    $datos .= '</p>';
    return $datos;
}
function reverso($vlrs, $registro)
{
    $datos = "<p>" . utf8_decode('En la ciudad de Guatemala, el día ') . fechletras($registro[0]['DFecDsbls']) . ' como notario DOY FE. </p>';
    $datos .= "<p>" . utf8_decode('Que las cinco firmas que anteceden son autenticas porque fueron puestas en mi presencia por las personas presentes, ') . ' ';
    $i = 0;
    $nombresdetalles = '';
    while ($i < count($registro)) {
    ...
    $datos .= utf8_decode('b) que las firmas relacionadas que estan contenidas en el presente pagaré por las señoras: ') . $nombresdetalles;
    $datos .= utf8_decode('. A favor de la ' . $vlrs[1] . ', compareciendo en el mismo en calidad de fiador solidario y mancomunado las señoras: <vb>');
...
function firmas($fpdf, $registro)
{
    ...
    $cont = 1;
    while ($i < count($registro)) {
        $fpdf->Cell(51, 6, strtoupper(utf8_decode($registro[$i]['short_name'])), 'T', 0, 'C');
        $fpdf->Cell(15, 6, ' ', '', 0);
        if ($cont > 2) {
            $fpdf->ln(25);
            $cont = 0;
