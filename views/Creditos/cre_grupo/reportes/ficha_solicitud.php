...
cre.CCODCTA,cre.Cestado,cre.NCiclo,cre.MontoSol,cre.CodAnal,concat(usu.nombre,' ',usu.apellido) nomanal,cre.CCODPRD,cre.CtipCre,cre.NtipPerC,cre.DfecPago,cre.noPeriodo,cre.Dictamen,cre.MonSug,cre.DFecDsbls,cre.DfecSol
...
WHERE cre.TipoEnti='GRUP' AND cre.CESTADO='A' AND cre.CCodGrupo='" . $archivo[0] . "'  AND cre.NCiclo=" . $archivo[1] . " ORDER BY cre.CCODCTA";
...
function printpdf($registro, $info)
{
    ...
    class PDF extends FPDF
    {
        ...
        function Header()
        {
            ...
            $fechasol = date("d-m-Y", strtotime($this->datos[0]["DfecSol"]));
            $this->Cell($ancho_linea, 7, $fechasol, '', 0, 'L');
...
function detallado($pdf, $registro)
{
    ...
    while ($fila < count($registro)) {
        ...
        $namecli = utf8_decode($registro[$fila]["short_name"]);
        $identificacion =  ($registro[$fila]["no_identifica"]==null) ? ' ' : $registro[$fila]["no_identifica"];
        $fecha =  ($registro[$fila]["date_birth"]==null) ? ' ' : date("d-m-Y", strtotime($registro[$fila]["date_birth"]));
