...
$query = "SELECT kar.CODKAR,kar.CCODCTA,cli.short_name,cli.no_identifica dpi,cli.no_tributaria nit,kar.DFECPRO,kar.NMONTO,kar.CNUMING,kar.KP,kar.INTERES,kar.MORA,kar.AHOPRG,kar.OTR,
    ...
    IFNULL((SELECT numcuenta FROM ctb_bancos where id=CCODBANCO),'-') CCODBANCO 
    FROM CREDKAR kar 
...
function printpdf($registro, $datos, $info)
{
    ...
    while ($fila < count($registro)) {
        ...
        $fechabanco = ($formapago == '0') ? $registro[$fila]["DFECBANCO"] : ' ';
        $nombrebanco = ($formapago == '0') ? $registro[$fila]["CBANCO"] : ' ';
        $cuentabanco = ($formapago == '0') ? $registro[$fila]["CCODBANCO"] : ' ';
        $formapago = ($formapago == '0') ? 'BOLETA DE BANCO' : 'EFECTIVO';
