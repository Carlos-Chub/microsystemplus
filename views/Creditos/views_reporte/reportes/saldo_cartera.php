...
//-------VALIDACIONES------------------[`ffin`],[`codofi`],[`rfechas`]

//*****************ARMANDO LA CONSULTA**************
$condi = "";
//RANGO DE FECHAS
$filtrofecha = ($radios[0] == "afecha") ? $inputs[0] : $hoy;
$titlereport = " AL " . date("d-m-Y", strtotime($filtrofecha));

$strquery = "CALL carterageneral('$filtrofecha')";
...
function printpdf($registro, $datos, $info)
{
    ...
    while ($fila < count($registro)) {
        ...
        $codgrupo = $registro[$fila]["CCodGrupo"];
...
function printxls($registro, $titlereport, $usuario)
{
    ...
    //total de registros
    $sum_monto = array_sum(array_column($registro, "MonSug"));
