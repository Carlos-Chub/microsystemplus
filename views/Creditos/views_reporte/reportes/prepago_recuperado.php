...
$texto_reporte = "Reembolsos de Montos debidos";

$query = "SELECT crem.ccodcta,cli.short_name,ag.nom_agencia,concat(us.nombre,' ',us.apellido) asesor,crem.TipoEnti tipenti,cuotas.*, 
IFNULL(cuotasxpag.capxpag,0) capxpag, IFNULL(cuotasxpag.intxpag,0) intxpag, IFNULL(cuotasxpag.otrxpag,0) otrxpag ,
IFNULL(pagos.cappag,0) cappag, IFNULL(pagos.intpag,0) intpag, IFNULL(pagos.otrpag,0) otrpag 
...
WHERE crem.Cestado IN ('F') AND acc IS NOT NULL " . $where . " ORDER BY ag.id_agencia,crem.CodAnal,crem.TipoEnti,crem.CcodGrupo, crem.ccodcta";
...
try {
    ...
    $result = $database->getAllResults($query, [$fecinicio, $fecfin, $fecinicio, $fecfin]);
...
function printxls($datos, $otros)
{
    ...
    $encabezado_tabla = ["CÓDIGO CUENTA", "NOMBRE CLIENTE", "AGENCIA", "ASESOR", "KP PACTADO", "KP RECUPERADO", "%", "INT PACTADO", "INT PAGADO", "%", "OTR PACTADO", "OTR RECUPERADO", "%", "TOTAL PACTADO", "TOTAL PAGADO"];
    ...
    while ($fila < count($datos)) {
        ...
        $porkp = ($ncapita > 0) ? ($cappag * 100 / $ncapita) : (($cappag > 0) ? 100 : $cappag);
        $porint = ($nintere > 0) ? ($intpag * 100 / $nintere) : (($intpag > 0) ? 100 : $intpag);
        $porotr = ($notros > 0) ? ($otrpag * 100 / $notros) : (($otrpag > 0) ? 100 : $otrpag);
        ...
        $hojaReporte->setCellValueByColumnAndRow(5, $linea, $ncapita);
        $hojaReporte->setCellValueByColumnAndRow(6, $linea, $cappag);
        $hojaReporte->setCellValueByColumnAndRow(7, $linea, $porkp);
        $hojaReporte->setCellValueByColumnAndRow(8, $linea, $nintere);
        $hojaReporte->setCellValueByColumnAndRow(9, $linea, $intpag);
        $hojaReporte->setCellValueByColumnAndRow(10, $linea, $porint);
        $hojaReporte->setCellValueByColumnAndRow(11, $linea, $notros);
        $hojaReporte->setCellValueByColumnAndRow(12, $linea, $otrpag);
        $hojaReporte->setCellValueByColumnAndRow(13, $linea, $porotr);
        $hojaReporte->setCellValueByColumnAndRow(14, $linea, $totproyectado);
        $hojaReporte->setCellValueByColumnAndRow(15, $linea, $totrecuperado);
        $fila++;
        $linea++;
    }
    $columnas = range('A', 'S');
...
function printpdf($datos, $otros, $info)
{
    ...
    while ($fila < count($datos)) {
        $cuenta = $datos[$fila]["ccodcta"];
        $nombre = strtoupper(utf8_decode($datos[$fila]["short_name"]));
        $agencia = strtoupper(utf8_decode($datos[$fila]["nom_agencia"]));
        $asesor = strtoupper(utf8_decode($datos[$fila]["asesor"]));
        $tipoenti = strtoupper(($datos[$fila]["tipenti"]));
        $ncapita = $datos[$fila]["ncapita"];
        $nintere = $datos[$fila]["nintere"];
        $notros = $datos[$fila]["otros"];
        ...
        $pdf->CellFit($ancho_linea, $tamanio_linea + 1, number_format($porkp, 2), 0, 0, 'R', 0, '', 1, 0);
        ...
        $pdf->CellFit($ancho_linea, $tamanio_linea + 1, number_format($porint, 2), 0, 0, 'R', 0, '', 1, 0);
        ...
        $pdf->CellFit($ancho_linea, $tamanio_linea + 1, number_format($porotr, 2), 0, 0, 'R', 0, '', 1, 0);
