...
cremi.Cdescre,
dc.DestinoCredito,
SUM(cremi.MonSug) AS suma_MonSug,
SUM(cremi.NCapDes) AS cantidad_Ncapdes,
SUM(IF(crepg.nmorpag > 1, 1, 0)) AS cantidad_mora,
...
LEFT JOIN 
`clhpzzvb_bd_general_coopera`.tb_destinocredito dc ON dc.id_DestinoCredito=cremi.Cdescre 
...
function printpdf($registro, $datos, $info, $no_creditos_global,$mora_global,$total_kap,$total_mora,$total_Cmora )
{
    ...
    class PDF extends FPDF
    {
        ...
        function Header()
        {
            ...
            $this->Cell($ancho_linea * 2 , 5, 'SECTOR', 'B', 0, 'L');
    ...
    while ($fila < count($registro)) {
        ...
        $nombre = utf8_decode($registro[$fila]["DestinoCredito"]);  
        $cantidad = utf8_decode($registro[$fila]["cantidad_registros"]);  
