...
$strquery = "SELECT
    ...
    WHERE cremi.DFecDsbls <= ? AND (cremi.CESTADO = 'F' OR cremi.CESTADO = 'G') AND (cremi.MonSug - IFNULL(kar.sum_KP, 0)) > 0 
    " . $filfondo . $filagencia . "
    HAVING atraso > 0
...
function printxls($registro, $titlereport, $usuario, $valida)
{
  ...
  while ($fila < count($registro)) {
    $idagencia = $registro[$fila]["id_agencia"];
    $nomagencia = utf8_encode($registro[$fila]["nom_agencia"]);
    $codanal = $registro[$fila]["CodAnal"];
    $nomanal =  utf8_encode($registro[$fila]["analista"]);
    ...
    if ($usuario == $codanal || $valida != "F0") {
      ...
      $genero =  $registro[$fila]["genero"];
      $direccion = (utf8_encode($registro[$fila]["direccion"]));
