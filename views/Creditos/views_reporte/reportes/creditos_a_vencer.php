...
WHERE cm.Cestado='F' AND (cm.DFecVen BETWEEN '" . $inputs[0] . "' AND '" . $inputs[1] . "') " . $filters . " ORDER BY ff.id,ag.id_agencia,us.id_usu,cm.CCodGrupo,cm.CCODCTA";
...
function printpdf($datos, $otros, $info)
{
    ...
    class PDF extends FPDF
    {
        ...
        function Header()
        {
            ...
            // $this->Ln(5);
            $this->CellFit($ancho_linea, $tamanio_linea + 1, utf8_decode("CRÉDITO"), 'B', 0, 'L', 0, '', 1, 0);
            $this->CellFit($ancho_linea * 2, $tamanio_linea + 1, utf8_decode('NOMBRE DEL CLIENTE'), 'B', 0, 'L', 0, '', 1, 0);
