...
{
    $data = utf8_decode('<p>En la ciudad de Guatemala departamento de Guatemala, NOSOTROS <vb>A) ' . $reprlegal . ', </vb>');
    $data .= utf8_decode('de cincuenta y cinco años de edad, casada, guatemalteca, administradora, con domicilio en el departamento de Guatemala, me identifico con el
    Documento Personal de Identificación (DPI) con Código Único de Identificación (CUI), ' . $dpireprletra . ' (' . $dpirepr . ') extendido por el Registro Nacional de las Personas ') .' -RENAP-'.utf8_decode( ' de la República de Guatemala, 
    ...
    $data .= utf8_decode('<vb>B)' . $name . '</vb> , de ' . $edadletra . ' años, ' . $estadocivil . ', guatemalteco, ' . $profesion . ', con domicilio en el departamento de 
    ...
    Personas') .' -RENAP-'.utf8_decode( ' de la República de Guatemala Centroamérica, en lo sucesivo del presente acto se me podrá denominar indistintamente como 
...
{
    $data = utf8_decode('<vb>PRIMERA.</vb> Por así convenirme yo, <vb>LA PARTE DEUDORA</vb> recibo para mi beneficio a título de MUTUO, de parte de la entidad ');
    $data .= utf8_decode('<vb>ASOCIACIÓN VIDA NUEVA</vb> la suma de ' . $montoletra . ',(' . $monto . '). <vb>SEGUNDA.</vb> Yo, <vb>LA PARTE DEUDORA, </vb>');
    $data .= utf8_decode('recibo la ya referida cantidad, con la obligación de invertirla única y exclusivamente para ' . $destinocredito . ', entendiendo que dentro de un plazo de DIEZ (10) días debo demostrar a <vb>LA PARTE ACREEDORA</vb> con la documentación respectiva que el dinero que 
    recibo en calidad de MUTUO se ha invertido en el proyecto anteriormente descrito.');
    $data .= utf8_decode('<vb>TERCERA: A) INTERESES:</vb>Ambas partes convenimos en el hecho que la suma de dinero entregada en calidad de mutuo, devengará un interés mensual 
...
{
    $data = utf8_decode('<vb>CUARTA: Yo, LA PARTE DEUDORA</vb> me obligo por este instrumento público a cancelar la suma mutuada en el plazo de 
    ...
    $data .= utf8_decode('<vb>LA PARTE DEUDORA </vb> deberá presentarse a las oficinas de la entidad <vb>ASOCIACIÓN VIDA NUEVA</vb>,');
    $data .= utf8_decode('ubicadas en la veintinueve (29) calle, siete guion cuarenta y dos, (7-42), zona tres (3) del Municipio y departamento de 
...
function puntos6()
{
    $data = utf8_decode('<vb>SEXTA: </vb> La <vb>PARTE ACREEDORA, </vb>a través de su representante legal, <vb>ACEPTA</vb> expresamente la garantía prendaria 
...
{
    $data = utf8_decode('<p>En la ciudad de Guatemala, el día ' . $fechaletras . ', como Notario 
    ...
    extendido por el Registro Nacional de las Personas ') .' -RENAP-'.utf8_decode( ' de la República de Guatemala, Centroamérica; y por la señora <vb>
    ' . $reprlegal . '</vb>, me identifico con el  Documento Personal de Identificación (DPI) con Código Único de Identificación (CUI), 
    número ' . $dpireprletra . ' (' . $dpirepr . ') extendido por el Registro Nacional de las Personas ') .' -RENAP-'.utf8_decode( ' de la República de');

    $data .= utf8_decode('Guatemala, Centroamérica, actúo en mi calidad de DIRECTOR EJECUTIVO Y REPRESENTANTE LEGAL de la entidad denominada 
