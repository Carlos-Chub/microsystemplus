...
$strquery = "SELECT cm.CCODCTA AS ccodcta, cm.DFecDsbls AS fecdesem, cm.TipoEnti AS formcredito, dest.DestinoCredito AS destinocred, cm.MonSug AS montoapro, cm.noPeriodo AS cuotas, tbp.nombre, cm.Dictamen AS dictamen,
    ...
    (IFNULL((SELECT dep.nombre FROM clhpzzvb_bd_general_coopera.departamentos dep WHERE dep.codigo_departamento = cl.depa_extiende),'-')) AS nomdepextiende,
    (IFNULL((SELECT mun.nombre FROM clhpzzvb_bd_general_coopera.municipios mun WHERE mun.codigo_municipio = cl.muni_extiende),'-')) AS nommunextiende,
    (IFNULL((SELECT DestinoCredito FROM clhpzzvb_bd_general_coopera.tb_destinocredito td WHERE td.id_DestinoCredito = cm.Cdescre),'-')) AS destinocredito
...
(IFNULL((SELECT dep.nombre FROM clhpzzvb_bd_general_coopera.departamentos dep WHERE dep.codigo_departamento = cg.depa),'-')) AS nomdep,
(IFNULL((SELECT mun.nombre FROM clhpzzvb_bd_general_coopera.municipios mun WHERE mun.codigo_municipio = cg.muni),'-')) AS nommun,
...
function printpdf($datos, $garantias, $clientefiador, $gastos, $info, $flag3, $conexion)
{
    ...
    $pdf->CellFit(0, $tamanio_linea, utf8_decode('RECONOCIMIENTO DE DEUDA CON GARANTÍA FIDUCIARIA EN DOCUMENTO PRIVADO POR '), 0, 0, 'C', 0, '', 1, 0);
    $pdf->Ln(5);
    $pdf->CellFit($ancho_linea+100, $tamanio_linea, utf8_decode(' Q' . number_format($datos[0]['montoapro'], 2, '.', ',') ), 0, 0, 'L', 0, '', 1, 0);

    $pdf->SetFont($fuente, 'I', $tamañofuente + 3);
    $pdf->CellFit($ancho_linea+20, $tamanio_linea, utf8_decode(' Contrato No. ' . $datos[0]['dictamen']), 0, 0, 'L', 0, '', 1, 0);
    ...
    $pdf->CellFit(0, $tamanio_linea, utf8_decode('ANTE MI'), 0, 0, 'C', 0, '', 1, 0);
