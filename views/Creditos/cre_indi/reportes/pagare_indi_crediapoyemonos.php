...
$pdf->CellFit(20, 5, ' ', 0, 0, 'L', 0, '', 1, 0);
$pdf->CellFit(77, 5, '(Fiador) ' . mb_strtoupper(utf8_decode($fiadores[0]['short_name'])), 0, 0, 'C', 0, '', 1, 0);
$pdf->Ln(5);
...
function parr1($datacre, $vlrs, $registro, $fiadores, $info, $creppg)
{
    ...
    $datos = (date("d", $fechadesembolso) == 1) ? utf8_decode("<p>Al primer día del mes") : utf8_decode("<p>A los $dia_desembolsoaux días del mes");
    $datos .= " de " . mb_strtolower($meses[date("m", $fechadesembolso) - 1], 'utf-8') . utf8_decode(' del año ') . utf8_decode($anodesembolsoaux);
    $datos .= ", en el municipio de " . utf8_decode($muniagencia) . ", departamento de " . utf8_decode($depaagencia);
    $datos .= ", YO, ";
    $i = 0;
    $nombresdetalles = '';
    while ($i < count($registro)) {
        $edadletrasaux = $edadletras->toWords((calcular_edad($registro[$i]['date_birth'])));
        $nombresdetalles .= '<vb>' . strtoupper(utf8_decode($registro[$i]['short_name'])) . '</vb>,' . utf8_decode(' de <vb>' . $edadletrasaux . ' AÑOS DE EDAD</vb>, ACTIVIDAD: <vb>' . utf8_decode(mb_strtoupper($registro[$i]['profesion']))) . ',</vb> Guatemalteco (a), con residencia en: ' . utf8_decode(mb_strtoupper($registro[$i]['Direccion'])) . ', ' . utf8_decode(mb_strtoupper($registro[0]['municipio'])) . ', ' . utf8_decode(mb_strtoupper($registro[0]['departamento'])) . ', <vb>me identifico con el Documento Personal de ' . utf8_decode('Identificación') . ' -DPI- con ' . utf8_decode('código único de identificación -cui- ') . $registro[$i]['no_identifica'] . ' extendido por el Registro Nacional de las Personas de la ' . utf8_decode('República de Guatemala,</vb>');
        $i++;
    }
    $datos .= $nombresdetalles;
    $datos .= ' Por medio del Presente ' . utf8_decode('Título de Crédito consistente en Pagaré libre de protesto') . ', <vb>PROMETO PAGAR INCONDICIONALMENTE</vb> a la orden de  <vb>CREDI APOYEMONOS, SOCIEDAD '.utf8_decode("ANÓNIMA").'</vb>, con domicilio en SEPTIMA AVENIDA, CANTON VITZAL, DIEZ GUION CERO TRECE (10-013), ZONA CERO, MUNICIPIO DE  ' . utf8_decode(mb_strtoupper($info[0]['municipioagencia'])) . ', DEPARTAMENTO DE ' . utf8_decode(mb_strtoupper($info[0]['departamentoagencia'])) . ', La cantidad de: ' . mb_strtoupper($montoletra . ' EXACTOS****(Q' . number_format($sumacapconinteres, 2)) . ') Mediante ' . $numerocuotasaux . ' (' . $registro[0]["noPeriodo"] . ') cuotas, mismas que ' . utf8_decode('haré') . ' efectivas en las fechas que ' . utf8_decode('más') . ' adelante se detallan, las cuales ' . utf8_decode('serán') . ' depositadas en la cuenta monetaria ' . utf8_decode('número') . ': <place>782-001873-2</place> del banco <place>Industrial</place> a nombre de <vb>CREDI APOYEMONOS, SOCIEDAD '.utf8_decode("ANÓNIMA").'.</vb></p>';
...
function parr2($registro)
{
    ...
    $datos .= '<p>' . utf8_decode('Las condiciones en que cumpliré con la obligación son las siguientes: ---------------------------------------');
    $datos .= '<place><vb>RENUNCIA AL FUERO DE SU DOMICILIO,</vb></place> el librador, <vb>renuncia en forma voluntaria al <place>fuero de sus respectivos domicilios y se somete a los tribunales que el tenedor de este ' . utf8_decode('pagaré') . ' elija.</place></vb> Acepto que la falta de pago de una sola cuota de las amortizaciones estipuladas, ' . utf8_decode('dará') . ' derecho al tenedor de este ' . utf8_decode('pagaré') . ' a dar por <vb>anticipado el plazo de la presente ' . utf8_decode('OBLIGACIÓN') . ' Y EXIGIR</vb> el pago del saldo adeudado y ' . utf8_decode('también si se dictare mandamiento de ejecución y/o embargo en mi contra. ESTE PAGARÉ SE EMITE LIBRE DE PROTESTO, LIBRE DE FORMALIDADES DE PRESENTACIÓN Y COBRO O REQUERIMIENTO. En caso de juicio, ni el tenedor de este pagaré ni los auxiliares que proponga deberán prestar garantía. En caso de remate servirá de base el avalúo o monto del adeudo o la primera postura a opción del tenedor de este pagaré. El deudor acepta como buenas, liquidas y exigibles las cuentas que el tenedor del pagaré presente, <vb>ASÍ COMO EL PRESENTE TITULO PERFECTO E INCONTESTABLE.</vb>') . '</p>';

    $datos .= '<p>UNICAMENTE SE PAGARA EL ' . $registro[0]['tasa_interes'] . '% DE INTERES ANUAL, SOBRE EL CAPITAL DE LA ' . utf8_decode('OBLIGACIÓN.') . ' POR INCUMPLIMIENTO de pago de cada una de las cuotas antes expresadas se pagara el ' . number_format($registro[0]['porcentaje_mora'], 2) . '% de intereses moratorios, ASI COMO EL 10.00% DE LOS COSTOS JUDICIALES QUE EL MISMO DERIVE AL MOMENTO DE SER EJECUTADO.</p>';
    return $datos;
}
