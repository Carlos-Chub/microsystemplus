...
printpdf($codigo, $conexion, $_SESSION['id_agencia']);

function printpdf($codigo, $conexion, $idagencia)
{
    ...
    WHERE cr.CCODCTA="' . $codigo . '" AND cr.CODKAR=' . $registro[0]["CODKAR"];
    ...
    $pdf->Ln(6);
    $pdf->Cell(110, $tamanio_linea, "NOMBRE: " . utf8_decode(mb_strtoupper($registro[0]["short_name"], 'utf-8')), 0, 0, 'L');
    $pdf->Cell(87, $tamanio_linea, "Q " . $registro[0]['NMONTO'], 0, 0, 'C');

    $pdf->Ln(9);
    $pdf->Cell(0, $tamanio_linea, utf8_decode(mb_strtoupper("RESEPCIÓN DE CREDITO A ", 'utf-8')) . utf8_decode(mb_strtoupper($registro[0]["short_name"], 'utf-8')) . ", PARA UN PERIODO", 0, 0, 'L');
    $pdf->Ln(6);
    ...
    $pdf->Cell(130, $tamanio_linea * 2, utf8_decode($montoletra), 0, 0, 'L');
    ...
    $pdf->Ln(4);
    $pdf->SetFont($fuente, 'B', 11);
    $pdf->CellFit(0, $tamanio_linea + 1,utf8_decode(utf8_decode('USUARIO: ' . $_SESSION['nombre'])). ' ' . utf8_decode(utf8_decode($_SESSION['apellido'])), 0, 0, 'C', 0, '', 1, 0);
