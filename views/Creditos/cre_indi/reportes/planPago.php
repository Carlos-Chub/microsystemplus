...
$ctipcre    = ($dtcre["CtipCre"]);
$short_name = ($dtcre["short_name"]);
$dfecpago   = ($dtcre["DfecPago"]);
$noperiodo  = ($dtcre["noPeriodo"]);
$ntipperc   = ($dtcre["NtipPerC"]);
$monsug     = ($dtcre["MonSug"]);
$interes    = ($dtcre["NIntApro"]);
$ccodcta    = ($dtcre["CCODCTA"]);
$ccodprod    = ($dtcre["CCODPRD"]);
...
$calcu = gastoscuota($ccodprod, $codcre, $conexion); 
$porCuota = $calcu;
...
if (($ntipperc == "1D" || $ntipperc == "7D") && $ctipcre == "Flat") {
    ...
    $amortiza = calculo_montos_diario($monsug, $interes, $noperiodo, $conexion);
    $fchspgs = calculo_fechas_por_nocuota($dfecpago, $noperiodo, $frecuencia, $conexion);
} else {
    ...
    // echo json_encode(['status' => 0, 'mensaje' => $noPeriodo]);
    // return;
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 5, 'Cliente : ' . utf8_decode(mb_strtoupper($this->compl_name, 'utf-8')), 0, 1, 'L');
...
//$pdf = new PDF();
$pdf = new PDF($ccodcta, $dfecpago, $short_name, $monsug, $interes, $dtcre2["descr"], $info, $porCuota);
$pdf->AliasNbPages();
$pdf->AddPage();
