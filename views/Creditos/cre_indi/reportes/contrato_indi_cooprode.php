...
if ($query) {
    ...
    if ($result) {
        ...
        $strquery_second = "SELECT DestinoCredito FROM clhpzzvb_bd_general_coopera.tb_destinocredito WHERE id_DestinoCredito = '$DESTCRED'";
...
function printpdf($datos, $garantias, $clientefiador, $gastos, $info, $flag3, $conexion)
{
    ...
    $pdf->CellFit(0, $tamanio_linea, utf8_decode('CONTRATO MUTUO CON GARANTIA FIDUCIARIA'), 0, 0, 'C', 0, '', 1, 0);
    $pdf->Ln(8);
  ...
  //  determinar si se imprime la segunda condicion
    ...
    $pdf->WriteTag(0, 4, utf8_decode($texto), 0, "C", 0, 0);
    $pdf->Ln(1);
...
function printpdf2($datos,  $info,  $conexion, $DESTINOCRED )
{
    ...
    //TITULO DE CONTRATO
    $pdf->SetFont($fuente, 'B', $tamañofuente + 5);
    $pdf->CellFit(0, $tamanio_linea, utf8_decode('CONTRATO DE MUTUO SIN GARANTIA'), 0, 0, 'C', 0, '', 1, 0);
    $pdf->Ln(8);
