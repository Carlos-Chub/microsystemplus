...
$txt = "";
$txt .= deudores($infoCredito, $conexion, $pdf);
...
function headermanual($fpdf, $dtCre, $info)
{
    ...
    // $fpdf->Cell(0, 4, 'MUNICIPIO:     '.utf8_decode($dtCre[4]), 0, 1, 'L');
    // $fpdf->Cell(130, 4, 'TASA POR MORA:   '.$dtCre[2].'%', 0, 0, 'L');
    // $fpdf->Cell(0, 4, 'BUENO POR :    ', 0, 1, 'L');
}

function deudores($data, $conexion)
{
    $fecha_actual = date("Y-m-d");
    $infoDeudor = dataCliente($data['cli1'], $conexion);
    $fiador = dataCliente($data['cli2'], $conexion);
    ...
    $datos = utf8_decode("<p><vb>Nosotros: a) " . $data['deudor1'] . "</vb>, me identifico con el Documento Personal de Identificación (DPI), con Código Único de Identificación (CUI) número " . $letra_dpi1 . ", " . $letra_dpi2 . ", " . $letra_dpi3 . " (" . $dpi_dividido[0] . " " . $dpi_dividido[1] . " " . $dpi_dividido[2] . ") extendido por el Registro Nacional de las Personas, de la República de Guatemala, Centroamérica; y");

    $datos .= utf8_decode("<vb> b) " . $data['fiador'] . "</vb>, me identifico con el Documento Personal de Identificación (DPI), con Código Único de Identificación (CUI) número " . $letra_dpi11 . ", " . $letra_dpi22 . ", " . $letra_dpi33 . " (" . $dpi_dividido1[0] . " " . $dpi_dividido1[1] . " " . $dpi_dividido1[2] . "), extendido por el Registro Nacional de las Personas, de la República de Guatemala, Centroamérica, a quienes en lo sucesivo se nos podrá denominar indistintamente como <vb>LOS DEUDORES</vb>.");

    $datos .= utf8_decode(" NOSOTROS, <vb>LOS DEUDORES,</vb> actuamos de manera mancomunada y solidaria, y por medio de este TÍTULO DE CRÉDITO <vb>PROMETEMOS PAGAR INCONDICIONALMENTE a la entidad ASOCIACIÓN VIDA NUEVA,</vb> la suma de " . $monto . " quetzales " . (($deciMonto == 0) ? "exactos" : "con " . $deciMonto) . " (Q " . $data['monto'] . "). Dicha suma de dinero será cancelada en " . numero_Letras($data['plazo']) . " (" . $data['plazo'] . ") amortizaciones <vb>" . tipoPeriodo($data['tipoPeriodo']) . "</vb> niveladas, sucesivas y consecutivas de");

    $datos .= utf8_decode(" " . numero_Letras($data['cuota']) . " quetzales " . ((decimal($data['cuota']) == 0) ? "exactos" : "con " . decimal($data['cuota'])) . " centavos" . " (Q " . $data['cuota'] . "), iniciándose el pago de la primera cuota a partir del día " . numero_Letras($fecha1['dia']) . " (" . $fecha1['dia'] . ") <vb>de " . $meses[intval(ltrim($fecha1['mes'], 0)) - 1] . "</vb> del año " . numero_Letras($fecha1['anio']) . " (" . $fecha1['anio'] . ") y finalizando el día " . numero_Letras($fecha2['dia']) . " (" . $fecha2['dia'] . ") <vb>de " . $meses[intval(ltrim($fecha2['mes'], 0)) - 1] . "</vb> del año " . numero_Letras($fecha2['anio']) . " (" . $fecha2['anio'] . ").");

    $datos .= utf8_decode(" El pago de las amortizaciones, se efectuará libre de impuestos, descuentos, retenciones o recargos presentes y futuros en cualquiera de las agencias del BANCO INTERNACIONAL, SOCIEDAD ANÓNIMA (INTERBANCO) ubicadas en todo el territorio de la República de Guatemala sin necesidad de cobro o requerimiento alguno.");

    $datos .= utf8_decode(" LOS DEUDORES deberán además presentarse a las oficinas de la entidad <vb>ASOCIACIÓN VIDA NUEVA</vb>, ubicadas en la veintinueve (29) calle, siete guion cuarenta y dos, (7-42), zona tres (3) del Municipio y departamento de Guatemala,");

    $datos .= utf8_decode(" para presentar la boleta de depósito en la que conste el pago de cada una de las amortizaciones " . tipoPeriodo($data['tipoPeriodo']) . " o enviarlas a través de la aplicación denominada WhatsApp a los números de teléfono institucionales siguientes:");

    $datos .= utf8_decode(" cuatro mil seiscientos noventa y cinco, siete mil trescientos treinta y cinco (4695 7335) y/o tres mil trescientos cuarenta y uno, nueve mil seiscientos cuarenta (3341 9640).");

    $datos .= utf8_decode(" Si <vb>LOS DEUDORES</vb> no realizan <place>el pago total de la cuota acumulada mensual,</place> dentro de los primeros cinco (5) días de cada mes, <vb>EL TENEDOR</vb> del presente Título de Crédito, cobrará a <vb>LOS DEUDORES</vb> en concepto de penalización por mora una tasa de interés variable del <vb>CINCO POR CIENTO (5%).</vb>");

    $datos .= utf8_decode("<vb> LOS DEUDORES</vb> reconocemos que, por el pago de esta penalización, no se entiende extinguida la obligación principal. Este recargo por mora, tendrá preferencia al pago de capital e intereses, en las amortizaciones realizadas al crédito. En caso de la falta de pago de dos de las cuotas, se dará por vencido el plazo y se podrá ejecutar la obligación contenida en el presente título de crédito. Asimismo, LOS DEUDORES renunciamos al fuero de nuestro domicilio, sometiéndonos expresamente a los tribunales que la entidad ASOCIACIÓN VIDA NUEVA, elija señalando como lugar para recibir notificaciones, emplazamientos, citaciones, avisos y/o diligencias en las siguientes direcciones:");

    $datos .= utf8_decode(" " . $infoDeudor['direc'] . ", municipio de " . $infoDeudor['mun'] . ", del departamento de " . $infoDeudor['dep'] . ", y " . $fiador['direc'] . ", municipio de " . $fiador['mun'] . " del departamento " . $fiador['dep'] . ", teniendo por válidas y bien hechas las notificaciones, comunicaciones y/o citaciones que en dicho lugar se nos  hicieren.");

    $datos .= utf8_decode(" Todos los gastos que directa o indirectamente ocasione el presente pagaré, serán por cuenta de <vb>LOS DEUDORES</vb>, incluyendo los de cobro judicial y/o extrajudicial, honorarios de abogado si fuere necesario, así como cualesquiera comisiones de cambio, impuestos, timbres fiscales, tasas y contribuciones, y cualquiera otros impuestos relacionados con este documento que existan actualmente o que se decreten en el futuro.");

    $datos .= utf8_decode(" Asimismo, en caso de acción judicial por parte del tenedor del presente pagaré, <vb>LOS DEUDORES</vb> renunciamos al derecho de exigir prestación de cualquier tipo de garantías y en caso de remate, servirá de base el monto del adeudo o la primera postura a opción del tenedor de este pagaré.");

    $datos .= utf8_decode("<vb>LOS DEUDORES aceptamos como buenas, líquidas y exigibles</vb> las cuentas que la tenedora de este pagaré presente. Este pagaré puede ser cedido, pignorado o enajenado parcial o totalmente y negociado en cualquier otra forma por su tenedor, sin necesidad de previo aviso ni posterior notificación a <vb>LOS DEUDORES.</vb>");

    $datos .= utf8_decode("</p>");

    $datos .= "<p></p>";

    $datos .= utf8_decode("<p> Guatemala, " . $fecha_actual['dia'] . " de " . $meses[intval(ltrim($fecha_actual['mes'], 0)) - 1] . " del año " . numero_Letras($fecha_actual['anio']) . ". </p>");
...
function dataCliente($codCli, $conexion)
{
    $sql = "SELECT tc.Direccion direc ,dep.nombre dep ,m.nombre mun FROM tb_cliente tc 
    INNER JOIN clhpzzvb_bd_general_coopera.departamentos dep ON dep.codigo_departamento = tc.depa_reside
    INNER JOIN clhpzzvb_bd_general_coopera.municipios m ON m.codigo_municipio = tc.muni_reside
...
function firmas($fpdf, $registro, $fiadores)
{
    ...
    $fpdf->Cell(60, 6, strtoupper(utf8_decode($registro)), 'T', 0, 'C');
    $x1 = $fpdf->Getx()+3;
    $y = $fpdf->Gety()-10;
    ...
    $fpdf->Cell(60, 6, strtoupper(utf8_decode($fiadores)), 'T', 0, 'C');
    $x2 = $fpdf->Getx()+3;
