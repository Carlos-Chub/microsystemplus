...
function recibo_crediapoyemonos($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea + 1, utf8_decode(utf8_decode($usuario)), 0, 0, 'L', 0, '', 1, 0);
    ...
    // $pdf->CellFit($ancho_linea2 + 53, $tamanio_linea + 1, 'NOMBRE:', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 3, $tamanio_linea + 1, utf8_decode(mb_strtoupper($registro[0][2], 'utf-8')), 0, 0, 'L', 0, '', 1, 0);
    ...
    $pdf->CellFit($ancho_linea2 * 4, $tamanio_linea + 1,  utf8_decode($format_monto->toMoney($decimal[0], 2, '', '')) . $res . "/100", 0, 0, 'L', 0, '', 1, 0);
