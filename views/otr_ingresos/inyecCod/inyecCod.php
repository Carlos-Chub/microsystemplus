...
?>
        <table class="table" id="otr_gastos">
            ...
            <tbody id="rep_otroGas">
                ...
                $consulta = mysqli_query($conexion, "SELECT  tp.id_nomenclatura AS idNom, tp.id, tp.nombre_gasto AS nombre, CONCAT(nom.ccodcta,' - ',nom.cdescrip) AS des, tp.tipo
                FROM otr_tipo_ingreso AS tp
                INNER JOIN ctb_nomenclatura AS nom ON nom.id = tp.id_nomenclatura WHERE tp.estado = 1");
                $con  =  0;
                ...
                ?>
                    ...
                    <tr>
                        ...
                        <td><?= $row['des'] ?></td>
                        <td>
                            <button type="button" class="btn btn-success" onclick="capData(['#idNom', '#idReg', '#gasto', '#nomenclatura', '#idSelect'], '<?= $data ?>');verEle(['#btnAct','#btnCan'], 1);verEle(['#btnGua'])"><i class="fa-solid fa-pen-to-square"></i></button>
                            <button type="button" class="btn btn-danger" onclick="eliminar('<?= $row['id'] ?>', 'eli_otrGasto', '<?= $codusu ?>')"> <i class="fa-solid fa-trash"></i></button>
                        </td>
                    </tr>
                <?php
                        ...
                        <div class="container mt-3">
                            ...
                            <table class="table" id="tbOtrosG">
                                ...
                                <tbody>
                                    ...
                                    <?php
                                    $consulta = mysqli_query($conexion, "SELECT id, nombre_gasto AS nomGasto FROM otr_tipo_ingreso WHERE estado = 1 AND tipo = $tipoIngreso");
                                    $con = 0;
                                    while ($row = mysqli_fetch_array($consulta, MYSQLI_ASSOC)) {
                                        $con += 1;
