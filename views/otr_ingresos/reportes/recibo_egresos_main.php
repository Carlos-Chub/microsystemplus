...
//*****************ARMANDO LA CONSULTA**************
// $consulta = "SELECT cl.idcod_cliente AS idcli, cl.short_name AS nomcli, clb.* FROM tb_cliente cl INNER JOIN tb_cli_balance clb ON cl.id_tb_cli_balance=clb.id WHERE cl.idcod_cliente='$archivo[0]'";
...
function printpdf($registro, $info, $id_usu, $conexion)
{
    ...
    $pdf->CellFit($ancho_linea2 - 25, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 20, $tamanio_linea + 1, $total, 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(7);
    ...
    $pdf->CellFit(0, $tamanio_linea + 1, 'ID:' . $id_usu . ' USUARIO:' . utf8_decode(utf8_decode($_SESSION['nombre'])) . ' ' . utf8_decode(utf8_decode($_SESSION['apellido'])), 0, 0, 'C', 0, '', 1, 0);
