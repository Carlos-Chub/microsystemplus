...
$stra = "";
if ($cha == "true") {
    $stra = " AND (`fecha_alta` BETWEEN '" . $ainicio . "' AND '" . $afin . "')";
}
$strb = "";
if ($chb == "true") {
    $strb = " AND (`fecha_baja` BETWEEN '" . $finicio . "' AND '" . $ffin . "')";
}
$strquery = "SELECT 
IFNULL((SELECT cdescri FROM clhpzzvb_bd_general_coopera.tn_EtniaIdioma WHERE Id_EtinIdiom=cli.idioma),'-') idiomades, age.nom_agencia,
cli.* FROM tb_cliente cli
INNER JOIN tb_agencia age on age.cod_agenc=cli.agencia
 WHERE `id_tipoCliente`='NATURAL' ";
if ($act == "0") {
    $strquery = $strquery . " AND `estado` = 'X' " . $stra . $strb;
} else {
...
while ($registro = mysqli_fetch_array($sql, MYSQLI_ASSOC)) {
    $idcod_cliente = utf8_encode($registro["idcod_cliente"]);
    $no_identifica = utf8_encode($registro["no_identifica"]);
    //$compl_name = utf8_encode($registro["primer_name"] . " " . $registro["segundo_name"] . " " . $registro["tercer_name"] . " " . $registro["primer_last"] . " " . $registro["segundo_last"]);
    $compl_name = utf8_encode($registro["short_name"]);
    $date_birth = date("d-m-Y", strtotime(utf8_encode($registro["date_birth"])));
    $idioma = utf8_encode($registro["idiomades"]);
    // $idioma = func_idiomas($etnia,$general)[0];
    $sexo = utf8_encode($registro["genero"]);
    $nit = utf8_encode($registro["no_tributaria"]);
    $direccion = utf8_encode($registro["Direccion"]);
    $telefono1 = utf8_encode($registro["tel_no1"]);
    $estadocivil = utf8_encode($registro["estado_civil"]);
    $agencia = utf8_encode($registro["nom_agencia"]);
    $profesion = ($registro["profesion"] == "") ? ' ' : utf8_encode($registro["profesion"]);
    $date_alta = ($registro["fecha_alta"] == null || $registro["fecha_alta"] == "") ? '0000-00-00' : date('d-m-Y', strtotime($registro["fecha_alta"]));
    // $date_baja = date("d-m-Y", strtotime(utf8_encode($registro["fecha_baja"])));
