...
while ($cligrupo = mysqli_fetch_array($queryCliGrupo, MYSQLI_ASSOC)) {
    $pdf->Ln(2);
    $codigocli = utf8_encode($cligrupo['idcod_cliente']);
    $nombrecli = utf8_encode($cligrupo['short_name']);
    $dpi = utf8_encode($cligrupo['no_identifica']);
    $fechanac = date("d-m-Y", strtotime(utf8_encode($cligrupo['date_birth'])));
