...
function cheque_bancoindustrial($pdf, $registro)
{
    ...
    $pdf->CellFit($ancho_linea + 80, $tamanio_linea * 2, utf8_decode("Nebaj Quiché, ")  . date("d", $fechaSegundos) . " DE " . $meses[date("n", $fechaSegundos) - 1] . " DE " . date("Y", $fechaSegundos), 'B', 0, 'L', 0, '', 1, 0);
    ...
    $pdf->SetFont($fuente, '', 12);
    $pdf->CellFit($ancho_linea * 4, $tamanio_linea * 2, "**** " . utf8_decode(mb_strtoupper($registro[0]['nomchq'], 'utf-8')) . " ****", 'B', 0, 'L', 0, '', 1, 0);
    ...
    $pdf->CellFit($ancho_linea * 5, $tamanio_linea * 2, "**** " . utf8_decode($format_monto->toMoney($decimal[0], 2, '', '')) . $res . "/100 ****", 'B', 0, 'L', 0, '', 1, 0);
    $pdf->CellFit(0, $tamanio_linea * 2, " ", 'R', 1, 'L', 0, '', 1, 0);
    ...
    $pdf->SetFont($fuente, '', 12);
    $pdf->MultiCell(0, 6, utf8_decode(mb_strtoupper($registro[0]['glosa'], 'utf-8')), 'LR', 'L');
    ...
    for ($i = 0; $i < count($registro); $i++) {
        ...
        $pdf->CellFit($ancho_linea * 3, $tamanio_linea + 1, utf8_decode(mb_strtoupper($registro[$i]['cdescrip'], 'utf-8')), 0, 0, 'L', 0, '', 1, 0);
        // $pdf->SetFont($fuente, 'B', 13);
    ...
    $pdf->CellFit($ancho_linea * 1.6, $tamanio_linea + 1, "Pedro de Leon Santiago", 'RL', 0, 'L', 0, '', 1, 0);
    ...
    $pdf->Ln(4);
    $pdf->SetFont($fuente, '', 11);
    $pdf->CellFit($ancho_linea * 1.6, $tamanio_linea + 1, "Gerente General", 'RL', 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea * 1.6, $tamanio_linea + 1, "Presidente Consejo de Administracion", 'RL', 0, 'L', 0, '', 1, 0);
    ...
    $pdf->CellFit($ancho_linea * 1.6, $tamanio_linea + 1, "DPI CUI 2562 41600 1413", 'RL', 0, 'L', 0, '', 1, 0);
