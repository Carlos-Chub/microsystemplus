...
class NumeroALetras
{
    ...
    public function toWords($number, $decimals = 2)
    {
        ...
        $numerocero="";
        if (strlen($number) > 1) {
            $numerocero = substr($number, 0, 1);
            if ($numerocero == '0') {
                $numerocero = $this->wholeNumber(0);
                $bandera=true;
            }
        }
        ...
        if ($bandera) {
            $splitNumber[0] = $numerocero . "" . $this->wholeNumber($splitNumber[0]);
        } else {
    ...
    public function toWords_plus($number, $decimals = 2)
    {
        $this->checkApocope();
