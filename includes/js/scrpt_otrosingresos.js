    ...
    success: function (data) {
      ...
      if (data2[1] == "1") {
        ...
        if (condi === "ins_otrGasto" || condi === "act_otrGasto") {
          ...
          printdiv('recibos_otros_ingresos', '#cuadro', 'otros_ingresos_01', '0');
        } else if (condi === "cre_otrRecibo") {
          $("#miForm")[0].reset();
          $("#total1").text("");
          //$('.table tbody').empty();table para resetear todas la tablas
          $(".tbRecibo-Reset tbody").empty();
          //Generacion de recibo
          var reprint = (archivo[2] == 1 )? 21 : 22;
          reportes([[], [], [], [data2[2]]], 'pdf', reprint, 0, 1);
        }
      } else {
...
function inyecCod(idElem, condi, extra = "0", url = "../views/otr_ingresos/inyecCod/inyecCod.php") {
  $.ajax({
    url: url,
    type: "POST",
    data: { condi, extra },
...
function CargarImagen(idinput, codigoimg) {
  ...
  if (codigoimg == '') {
    Swal.fire({ icon: 'error', title: '¡ERROR!', text: 'No se ha seleccionado un recurso' });
    return;
  }
