    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data2[1] == "1") {
                ...
                if (condi == "update_user"|| data2[0]=="Contraseña Actualizada" ) {
                    if (data2[2] == "1") {
...
// funciones prueba para ver las
	function passrevisar(inputs){
		// se debera de obtener los datos y validar que sean iguales
		const inputs2 = [''];    var i = 0;
    while (i < inputs.length) {
    	inputs2[i] = document.getElementById(inputs[i]).value;
    	i++;
    }
		const inpass_new1 = inputs2[1];
		const inpass_new2 = inputs2[2];
		// Validar que las nuevas contraseñas sean iguales
		if (inpass_new1 !== inpass_new2) {
			Swal.fire({ icon: 'info', title: 'alerta!', text: "Contraseñas NO Coinciden" });
			// Si Contraseñas NO Coinciden, deshabilitar el botón
			document.getElementById('miBoton').disabled = true;
			return;
		}
		// Aquí puedes realizar otras acciones si es necesario
		Swal.fire({ icon: 'success', title: 'Muy Bien!', text: "Contraseñas Coinciden" });
		// Si Contraseñas Coinciden, habilitar el botón
